...
def score_faithfulness(
    ...
    # Kiểm tra xem câu trả lời có phải là từ chối do thiếu thông tin không
    refusal_keywords = [
        "không tìm thấy",
        "không có thông tin",
        "không đủ thông tin",
...
CRITICAL INSTRUCTION: If the answer correctly states that it cannot answer due to lack of information in the context, you MUST give it a score of 5, because it is being truthful and not hallucinating.

Output only valid JSON: {{"score": <int>, "reason": "<string>"}}"""

    try:
        response_text = call_llm_eval(prompt)
        ...
        if is_refusal:
            return {
                "score": 5,
                "notes": "Fallback heuristic: Answer is a refusal due to lack of info, which is faithful.",
            }
        ...
        if is_refusal:
            return {
                "score": 5,
                "notes": f"Fallback heuristic due to judge error ({e}): Answer is a refusal.",
            }
        heuristic = _to_1_5(_overlap_ratio(answer, chunks_text))
        return {
            "score": heuristic,
...
def score_answer_relevance(
    ...
    """
    # TODO Sprint 4: Implement score_answer_relevance
    if answer in ["PIPELINE_NOT_IMPLEMENTED"] or answer.startswith("ERROR"):
        return {"score": 1, "notes": "No valid answer"}
...
CRITICAL INSTRUCTION: If the answer correctly states that the information is not available to answer the query, you MUST give it a score of 5, because it appropriately addresses the query's unanswerability.
...
Output only valid JSON: {{"score": <int>, "reason": "<string>"}}"""
    ...
    try:
        ...
        if is_refusal:
            return {
                "score": 5,
                "notes": "Fallback heuristic: Answer is a refusal, which is relevant to an unanswerable query.",
            }
...
def score_completeness(
    ...
    """
    # TODO Sprint 4: Implement score_completeness (so sánh với expected_answer)
    ...
    is_answer_refusal = any(kw in answer.lower() for kw in refusal_keywords)
    is_expected_refusal = any(kw in expected_answer.lower() for kw in refusal_keywords)

    prompt = f"""Compare the model answer with the expected answer for this query.
Query: {query}
Model answer: {answer}
...
CRITICAL INSTRUCTION: If both the model answer and expected answer state that the information is not available, you MUST give it a score of 5.
...
Output only valid JSON: {{"score": <int>, "reason": "<string>"}}"""
    ...
    try:
        ...
        # Fallback heuristic
        if is_answer_refusal and is_expected_refusal:
            return {
                "score": 5,
                "notes": "Fallback heuristic: Both answers are refusals.",
            }
        ...
        if is_answer_refusal and is_expected_refusal:
            return {
                "score": 5,
                "notes": f"Fallback heuristic due to judge error ({e}): Both answers are refusals.",
            }
        heuristic = _to_1_5(_overlap_ratio(expected_answer, answer))
        return {
            "score": heuristic,
...
def run_scorecard(
    ...
    for q in test_questions:
        ...
        try:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if transient and attempt < max_attempts:
                        if verbose:
                            print(
                                f"  Retry {attempt}/{max_attempts - 1} do lỗi tạm thời: {e}"
                            )
            ...
            if result is None:
                raise RuntimeError(
                    last_error if last_error else "Unknown pipeline error"
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Pipeline Variant chưa implement.")
        variant_results = []
